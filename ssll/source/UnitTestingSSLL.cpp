// UnitTestingSSLL.cpp : Defines the entry point for the console application.
//
...
TEST_CASE("Testing methods that do not change the list"){    
	...
	SECTION("Contains()"){
		REQUIRE(list.contains(25,&SSLL<int>::equals_function) == true);
		REQUIRE(list.contains(50, &SSLL<int>::equals_function) == false);
		REQUIRE(list.contains(0, &SSLL<int>::equals_function) == true);
		REQUIRE(list.contains(49, &SSLL<int>::equals_function) == true);
	}
...
TEST_CASE("Iterator testing"){
	...
	SSLL<int>::iterator begin = list.begin();
	SSLL<int>::iterator end = list.end();
	...
	//const iterator
	const SSLL<int> l=list;
	SSLL<int>::const_iterator b=l.begin();
	SSLL<int>::const_iterator e=l.end();
...
SSLL<int> list;
	...
	const SSLL<int> l(list);       //copy constructor for list class;
	SSLL<int>::const_iterator b=l.begin();
	SSLL<int>::const_iterator e=l.end();
...
TEST_CASE("List/const List [] operator"){
	SSLL<int> list;
	for (int i = 0; i < 50; i++){
		list.push_back(i);
	}
